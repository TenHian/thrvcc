...
int main()
{
	...
	printf("[125] support for func return short\n");
	ASSERT(1, true_fn());
	ASSERT(0, false_fn());
	ASSERT(3, char_fn());
	ASSERT(5, short_fn());
	...
	// [145] raise float to double if func parameter type is not specified
	printf("[145] raise float to double if func parameter type is not specified\n");
	ASSERT(0, ({
		       char buf[100];
